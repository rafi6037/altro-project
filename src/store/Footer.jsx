...
export default function Footer() {
  ...
  const acceptedPaymentMethods = [
    ...
    { key: 'nagad', label: 'Nagad', logo: nagadLogo, enabled: toBool(getSetting('nagad_enabled'), true) },
    { key: 'rocket', label: 'Rocket', logo: rocketLogo, enabled: toBool(getSetting('rocket_enabled'), false) },
    { key: 'bank', label: 'Bank Transfer', logo: bankLogo, enabled: toBool(getSetting('bank_enabled'), false) },
    { key: 'cod', label: 'Cash on Delivery', logo: cashLogo, enabled: toBool(getSetting('cod_enabled'), true) },
              ...
              <div className="flex gap-3">
                ...
                {facebookUrl ? (
                  ...
                  <span
                    ...
                    className="p-2.5 rounded-lg bg-white/5 opacity-50 cursor-not-allowed"
                  >
          ...
          <div className="mt-10 pt-6 border-t border-white/10">
            ...
            <div className="flex flex-nowrap items-center gap-2 sm:gap-3 overflow-x-auto pb-1">
              {acceptedPaymentMethods.map((method) => (
                <div
                  key={method.key}
                  className="h-9 sm:h-10 w-16 sm:w-[70px] shrink-0 bg-white rounded-lg border border-white/10 flex items-center justify-center px-1.5 sm:px-2"
                >
                  <img
                    src={method.logo}
                    alt={method.label}
                    className="max-h-6 sm:max-h-7 w-full object-contain"
                  />
                </div>
              ))}
            </div>
